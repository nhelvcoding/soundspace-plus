import { getOwnedItems, ITEM_DATA, equipItem, getEquipped } from "./globals.js";
...
const ownedKeys = getOwnedItems();
const items = ownedKeys.map(key => ({
  key,
  name: ITEM_DATA[key]?.name || key,
  type: ITEM_DATA[key]?.type || "misc"
}));
...
function drawCard(item, x, y, w, h, hover) {
  const equipped = getEquipped(item.type) === item.key;
  ...
  ctx.strokeStyle = equipped ? "lime" : hover ? "cyan" : "white";
  ctx.lineWidth = equipped ? 5 : hover ? 4 : 2;
  ...
  if (equipped) {
    ...
    ctx.fillStyle = "lime";
    ctx.fillText("(Equipped)", x + w / 2, y + h * 0.75);
  }
}
